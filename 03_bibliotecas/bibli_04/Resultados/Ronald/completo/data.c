...
void imprimeProximaData(int dia, int mes, int ano) {
    int novoDia = dia, novoMes = mes, novoAno = ano;

    novoDia += 1;

    // Se ultrapassar o numero de dias permitido
    if(novoDia > numeroDiasMes(mes, ano)) {
        novoDia = 1;
        novoMes += 1;

        // Se ultrapassar o numero de meses permitido
        if(novoMes > 12) {
            novoMes = 1;
            novoAno += 1;
        }
    ...
    imprimeData(novoDia, novoMes, novoAno);
}